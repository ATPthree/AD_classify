...
# # 定义一些参数，模型选择了最基础的bert英文模型
batch_size = 2   # 批大小
epoches = 100    # 训练轮数
model = "D:/111bertmodel/bertmodel" # bert模型地址
hidden_size = 768  # bert的隐藏层大小
n_class = 2        # 积极和消极
maxlen = 8      # 句子的最大长度

encode_layer=12 # bert的层数
filter_sizes = [2, 2, 2] # 3个卷积核
num_filters = 3      # 每种卷积核的个数

# data，构造一些训练数据
#sentences = ["我喜欢打篮球", "这个相机很好看", "今天玩的特别开心", "我不喜欢你", "太糟糕了", "真是件令人伤心的事情"]

sentences = ["I like playing basketball",
...
def main():
    ...
    for epoch in range(epoches):
      for i, batch in enumerate(train_loader):
        ...
        if epoch % 10 == 0:
          torch.save(
            bert_blend_cnn.textcnn.state_dict(),
            os.path.join(save_dir,f'textcnn_epoch{epoch + 1}.pth')
          )
    ...
    bert_blend_cnn.eval()
    with torch.no_grad():
      test_text = ['I hate playing basketball']
      ...
      if pred[0][0] == 0:
        ...
        print('积极')
    torch.save(bert_blend_cnn.textcnn.state_dict(),
               os.path.join(save_dir,'final_textcnn_weights.pth')
               )
    print('模型保存成功')
